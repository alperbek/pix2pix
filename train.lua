...
opt = {
   ...
   print_freq = 400,             -- print the debug information every print_freq iterations
...
function defineD(input_nc, output_nc, ndf)
    ...
    use_sigmoid = opt.use_wgan ~= 1
    if     opt.which_model_netD == "basic" then netD = defineD_basic(input_nc_tmp, output_nc, ndf, use_sigmoid)
    elseif opt.which_model_netD == "n_layers" then netD = defineD_n_layers(input_nc_tmp, output_nc, ndf, opt.n_layers_D, use_sigmoid)
...
local fGx = function(x)
  ...
  if opt.use_L1==1 then
    errL1 = criterionAE:forward(fake_B, real_B) * opt.lambda
